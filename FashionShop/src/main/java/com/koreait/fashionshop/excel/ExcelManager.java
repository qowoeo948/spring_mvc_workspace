...
public class ExcelManager {
	...
	public ExcelManager() {
		...
		try {
			fis=new FileInputStream("C:/spring-workspace/FashionShop/src/main/webapp/resources/excel/myskil.xlsx");
			...
			//이 시점부터는 sheet를 제어할 수 있으므로, 총 몇건이 존재하는지 알아본다.
			...
			for(int i=0;i<rows;i++) {
				...
				for(int a=0;a<columCount;a++) {
					XSSFCell cell=row.getCell(a);	//컬럼한개를 접근
